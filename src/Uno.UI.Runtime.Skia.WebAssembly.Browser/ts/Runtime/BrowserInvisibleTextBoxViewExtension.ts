namespace Uno.UI.Runtime.Skia {
	export class BrowserInvisibleTextBoxViewExtension {
		...
		// Text a compositionend delivered before the input event that puts it in place of the preedit
		// (WebKit's order); the composition completes once that event has synced the value.
		private static pendingCommit: string | null = null;
		// The preedit was removed by an input event of WebKit's own, on its way to putting the committed
		// text in place; the removal is reported with what follows it.
		private static preeditRemoved: boolean = false;
		// The composition update last reported, so the committed text put back where the preedit was
		// is not reported as a change.
		private static reportedText: string | null = null;
		private static reportedStart: number = -1;
		...
		private static createInput(isPasswordBox: boolean, text: string, acceptsReturn: boolean, inputMode: string, enterKeyHint: string, isReadOnly: boolean) {
			...
			input.addEventListener("compositionstart", () => {
				...
				BrowserInvisibleTextBoxViewExtension.reportedStart = -1;
		...
		private static syncFromInput(input: HTMLInputElement | HTMLTextAreaElement, ev: InputEvent | null) {
			if (BrowserInvisibleTextBoxViewExtension.isComposing) {
				...
				if (text.length > 0 && ((removed && !reinserted) || !input.value.startsWith(text, start))) {
					if (pendingCommit !== null && !isCompositionInput) {
				...
				if (BrowserInvisibleTextBoxViewExtension.isComposing) {
					...
					if (text !== BrowserInvisibleTextBoxViewExtension.reportedText
						|| start !== BrowserInvisibleTextBoxViewExtension.reportedStart
						|| input.value !== BrowserInvisibleTextBoxViewExtension.lastSyncedValue) {
						BrowserInvisibleTextBoxViewExtension.reportedText = text;
						BrowserInvisibleTextBoxViewExtension.reportedStart = start;
						BrowserInvisibleTextBoxViewExtension._imeExports.OnCompositionUpdated(text, start, input.value !== BrowserInvisibleTextBoxViewExtension.lastSyncedValue);
					}
				}
			...
			if (pending !== null && BrowserInvisibleTextBoxViewExtension.isCurrentInput(input) && BrowserInvisibleTextBoxViewExtension.isComposing
				&& (pending.length === 0 || input.value.startsWith(pending, BrowserInvisibleTextBoxViewExtension.compositionStart))) {
